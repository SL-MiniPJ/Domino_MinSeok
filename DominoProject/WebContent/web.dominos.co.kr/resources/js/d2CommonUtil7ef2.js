...
//챗봇 팝업
function domichat_open(is_app){
		
	var auth = cookieManager.getCookie("AUTH");
	var chat_boot_check = chatbotcheck();
	
	//챗봇 오픈 상태일때만 오픈 
	if (chat_boot_check == 'Y') {
...
//챗봇 오픈 타입
function chatbot_open_type(is_app){
	if (is_app == 'N') {
		window.open("https://domichat.dominos.co.kr/", "test_domichat", "width=433, height=830, left=100, top=50, resizable=yes, scrollbars=yes");
	} else {
		location.href = "https://domichat.dominos.co.kr/";
	}
}
	...
	var data = {serverId:serverId};
		 $.ajax({
				...
				success:function(data) {
					if(data.code_val=="0"){
						alert("도미챗 Season 1 종료\n\n그동안 도미챗을 이용해주셔서 감사합니다.\n\n더 똑똑해진 도미챗 Season2로\n찾아뵙겠습니다.");
						rtnVal = "N";
					}else{
						rtnVal = "Y";
					}
