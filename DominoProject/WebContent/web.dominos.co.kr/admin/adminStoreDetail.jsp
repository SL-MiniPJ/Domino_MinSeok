	...
	<div id="container">
    	<section id="content">
    		...
    		<div id="store_list" align="center">
    			...
    			<h1 class="page-title" style="font-size: 36px; padding: 50px">도미노 피자 전국 매장 리스트</h1>
	    		<table style="margin-bottom: 30px">
	    			...
	    			<tr>
	    				<th>매장 주소</th>
	    				<td><%=bean.getStore_address() %></td>
	    			</tr>
	    			<tr>
	    				<th>매장 위치</th>
	    				<td><%=bean.getStore_location() %></td>
	    			</tr>
	    			<tr>
	    				<th>매장 전화번호</th>
	    				<td><%=bean.getStore_phone() %></td>
	    			</tr>
	    			<tr>
	    				<th>개점일</th>
	    				<td><%=bean.getStore_opening() %></td>
	    			</tr>
	    			<tr>
	    				<th>주차 정보</th>
	    				<td>
	    					<%if(bean.getStore_park() == null) {%>
	    						-
	    					<%} else {%>
	    						<%=bean.getStore_park() %>
	    					<%} %>
	    			...
	    			<tr>
	    				<th>온라인 방문 포장</th>
	    				<td>
	    					<%if(bean.getStore_online() == 0) {%>
	    						-
	    					<%} else {%>
	    						<%=bean.getStore_online() %>%할인
	    					<%} %>
	    				</td>
	    			</tr>
	    			<th>오프라인 방문 포장</th>
	    				<td>
	    					<%if(bean.getStore_offline() == 0) {%>
	    						-
	    					<%} else {%>
	    						<%=bean.getStore_offline() %>%할인
	    					<%} %>
	    				</td>
	    			</tr>
	    			<th>특별 할인</th>
	    				<td>
	    					<%if(bean.getStore_sale() == 0) {%>
	    						-
	    					<%} else {%>
	    						<%=bean.getStore_sale() %>%할인
	    					<%} %>
	    				</td>
	    			</tr>
    		...
    		<div class="btn-wrap">
				<a href="./AdminStoreModifyView.st?store_code=<%=bean.getStore_code() %>" class="btn-type v6" style="margin-bottom: 100px">정보 수정</a>
			</div>
    	</section>
	</div>
